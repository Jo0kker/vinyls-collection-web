...
const ButtonAddVinyl = ({collectionId}: { collectionId: number }) => {
  ...
  const searchVinyls = async (page = 1) => {
    ...
    if (discogCode) {
      ...
      const discogId = discogCode.replace('[', '').replace(']', '').replace('m', '').replace('r', '')
          ...
          {indexStep !== 2 ? (
            <Accordion openIndex={openIndex} onToggle={handleToggle}>
              ...
              >
                ...
                >
                  <span className="italic opacity-40 text-center w-full">
                  ...
                  <Tooltip
                    content={
                      <Image src={"https://vinyls-collection.fra1.cdn.digitaloceanspaces.com/helperCodeDiscog.png"}
                      ...
                      >
                        ...
                        <div className="col-span-2 flex flex-col self-center">
                          <div className="flex flex-row ">
                            {item.discog_id && (
                              <Tooltip content="Vinyle importé de Discogs" placement="bottom">
                                <FontAwesomeIcon icon={faTurntable} color="purple" size="xl" />
                              </Tooltip>
                            )}
                          ...
                          {/* if item.artist or item.artists.name join */}
                            { item.artist ? item.artist : item.artists?.map((artist: { name: string }) => artist.name).join(', ') }
                          </h3>
                        ...
                        onClick={() => {
                          if (indexStep === 0) {
                            ...
                            searchDiscogs(1).then(r => {
                              ...
                              showToast({
                                type: 'error',
                                message: "Identifiant Discogs invalide"
                              })
                            })
                          } else if (indexStep === 1) {
                            setIndexStep(2)
