...
const Collector = ({ users, meta }: { users: User[], meta: {
    current_page: number,
    last_page: number,
    total: number
  } }) => {
        ...
        {collectors.map((user) => (
            <Link key={user.id} href={`/users/${user.id}`} className={"flex flex-row m-1 border border-gray-300 rounded border-8 hover:bg-gray-400"}>
              ...
              <div className={'flex flex-col mx-3 justify-center'}>
                ...
                <p className={'text-sm'}>Dernière connection : {DateTime.fromFormat(user.last_activity, 'yyyy-MM-dd HH:mm:ss').toFormat('dd/MM/yyyy')}</p>
              </div>
            </Link>
          ))}
      </div>
